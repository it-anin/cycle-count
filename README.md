...
ใช้งานจริงด้วยเครื่อง PDA 5 เครื่อง — 4 เครื่องเป็นของพนักงานนับ อีกเครื่องแอดมินใช้

**ไม่มีการผูกบัญชีกับเครื่อง** ใครหยิบเครื่องไหนไปก็ล็อกอินด้วยรหัสตัวเองได้
ระบบแยกผลนับที่ระดับ **คน** ไม่ใช่ระดับเครื่อง (`count_lines.counted_by`)
สลับเครื่องกันกลางกะจึงไม่มีปัญหา และ audit trail ยังบอกได้ว่าใครนับ ไม่ใช่เครื่องไหนนับ

### รหัสพนักงานเป็นภาษาไทยไม่ได้

`authEmailForEmployee()` ตัดทุกอย่างที่ไม่ใช่ `a–z0–9` ทิ้ง `"มุก"` จึงเหลือค่าว่างแล้ว throw
**ชื่อที่แสดงบนจอเป็นไทยได้ตามปกติ** — ข้อจำกัดอยู่ที่รหัสล็อกอินอย่างเดียว

```bash
pnpm --filter @cycle-count/db create:user -- \
  --code MUK --name "มุก" --pin 123456 --warehouse "คลังสินค้า" --role counter
```

รันคำสั่งเดิมซ้ำ = เปลี่ยน PIN (ไม่ได้สร้างบัญชีใหม่)

### เปิด/ปิดบัญชี

```bash
pnpm --filter @cycle-count/db user:active -- --list           # ดูทั้งหมด
pnpm --filter @cycle-count/db user:active -- --code MUK --off # ปิด
pnpm --filter @cycle-count/db user:active -- --code MUK --on  # เปิด
```

ปิดแล้วทุก endpoint ตีกลับ 403 ทันที **ข้อมูลที่นับไว้ไม่หาย** (`counted_by` เป็น uuid เปล่า
ไม่มี FK ไป `profiles` จึงไม่มี cascade)

> ⚠ การเช็ค `active` อยู่ที่ `requireUser()` ใน `apps/web/src/server/auth.ts` **ไม่ใช่ที่ RLS**
> เพราะ Drizzle ต่อ DB ด้วย role `postgres` ซึ่ง BYPASSRLS — `cc_is_active_user()` ใน migration
> จึงไม่เคยทำงานกับ query ที่ผ่านแอป ถ้าย้ายการเช็คออกจากตรงนั้นเมื่อไร ช่องโหว่จะกลับมาทันที

### สองข้อที่ต้องบอกทีมก่อนเริ่มนับ

**1. ห้ามสองคนนับชั้นเดียวกัน** ระบบรวมยอดของทุกคนเข้าด้วยกันตามการออกแบบ (เพื่อให้แบ่งโซนกันนับได้)
ถ้าเผลอนับชั้นซ้ำ ยอดจะถูกบวกสองรอบแล้วขึ้นเป็น "เกิน"

**ระบบกันให้ไม่ได้** เพราะยอดตั้งต้นจาก `public.stock` เป็นยอดรวมทั้งคลัง ไม่มีข้อมูลระดับชั้นวาง
ต้องแบ่งโซนกันเองก่อนเริ่ม

**2. ใบเฉลยบน PDA รวมยอดของทุกคน** ถ้ามุกนับ SKU หนึ่งได้ 7 แผงจากโซนตัวเอง แต่ทั้งคลังมี 62
ใบเฉลยจะขึ้นว่าขาด 55 ทั้งที่มุกไม่ได้ทำอะไรผิด — ของที่เหลืออยู่ในโซนคนอื่นที่ยังไม่ได้นับ

พอแล็คนับต่ออีก 5 ใบเฉลยของแล็คจะขึ้น "นับได้ 12" คือยอดรวมของทั้งสองคน
**ตัวเลขนี้มีความหมายก็ต่อเมื่อทุกคนนับ SKU นั้นครบแล้ว**

### หน้าแอดมินเมื่อมีหลายคนนับ

แถบ "ผู้นับ" จะโผล่ใต้แถบกรอง แสดงว่าแต่ละคนนับไปกี่ SKU และล่าสุดเมื่อไร
กดชิปแล้วกรองตารางเหลือเฉพาะแถวที่คนนั้นมีส่วนนับ (ตัวเลขบนชิปกรองขยับตามด้วย)

**แถบนี้โผล่เฉพาะตอนมีคนนับมากกว่าหนึ่งคน** — รอบที่มีคนเดียวมันไม่ได้บอกอะไรเพิ่ม
และหน้านี้เลือกเลย์เอาต์แบบตารางเต็มจอมาด้วยเหตุผลเดียวคือความหนาแน่น

ไฟล์ Excel ที่ส่งออกมีชีต **สรุปรายคน** เพิ่มมาให้ด้วย

> ช่อง "จำนวน SKU" ในแถวรวมนับแบบไม่ซ้ำ ถ้าสองคนนับตัวเดียวกันจะน้อยกว่าผลบวกรายคน — ไม่ใช่คำนวณผิด

## ประหยัดแบตบนเครื่อง PDA

ไล่โค้ดฝั่ง PDA ทั้งหมดแล้ว **ตัวกินแบตหลักไม่ได้อยู่ในแอป** — อยู่ที่จอกับหัวสแกน
ซึ่งแอปคุมไม่ได้ ที่แอปทำได้จึงมีจำกัด และการตั้งค่าเครื่องให้ผลมากกว่าการแก้โค้ดหลายเท่า

### สิ่งที่แอปทำไว้แล้ว

- **ไม่ยิง HTTP ต่อการสแกน** — catalog ทั้งรอบอยู่ใน IndexedDB ค้นจาก `Map` ในหน่วยความจำ
  ตอนเดินนับ radio จึงเงียบสนิท นี่คือตัวประหยัดที่ใหญ่ที่สุดของสถาปัตยกรรมนี้
- ไม่มี wake lock / `keepScreenOn` — จอดับตามค่าระบบ
- permission มีแค่ `INTERNET`
- ไม่มี polling, ไม่มี websocket realtime
- CSS ไม่มี `@keyframes` / `transition` / `filter` เลย
- BroadcastReceiver ลงทะเบียนเฉพาะตอนแอปอยู่หน้าจอ
- เขียน localStorage แบบหน่วง 800 ms แทนทุกครั้งที่ state เปลี่ยน (ดู `SAVE_DEBOUNCE_MS`)
- แถวในสมุดห่อ `memo()` — ยิงหนึ่งครั้งตอนมี 300 SKU วาดใหม่ 2 แถว ไม่ใช่ 300

### การตั้งค่าเครื่อง — เรียงตามผลจริงมากไปน้อย

**1. จอคือตัวกินแบตอันดับหนึ่ง** ตั้ง timeout 30 วินาที ความสว่างอัตโนมัติหรือราว 40%
การยิงบาร์โค้ดปลุกจอเองอยู่แล้ว ไม่ต้องตั้งให้จอค้างสว่าง

**2. ปิด "Wi-Fi scanning always available"** (ตั้งค่า → ตำแหน่ง → การสแกน Wi-Fi)
ในคลังที่มี AP หลายตัว การสแกนหา AP เบื้องหลังกับการ roam ไปมาคือตัวกินจริง

**3. ปิด Bluetooth / NFC / Location** ถ้าไม่ได้ใช้

**4. เปิด battery saver ได้** แอปนี้ไม่มีงานเบื้องหลัง จึงไม่มีอะไรให้ระบบไปหยุด
แต่ต้องลองก่อนว่าไม่ไปกระทบ service ของหัวสแกน

**5. ปิด Wi-Fi ระหว่างเดินนับได้เลย** — แอปทำงานออฟไลน์เต็มรูปแบบหลังโหลด catalog แล้ว
เปิด Wi-Fi เฉพาะตอนจะกดส่ง วิธีนี้ตัด radio ออกจากสมการทั้งกะ

> ⚠ ข้อ 5 ยังไม่ได้ทดสอบกับการทิ้งไว้เกิน 1 ชั่วโมง — access token อายุ 3,600 วินาที
> ถ้าออฟไลน์นานกว่านั้นต้องให้ supabase-js ต่ออายุสำเร็จก่อนถึงจะส่งได้
> **ทดสอบให้ผ่านก่อนแนะนำพนักงานใช้จริง**

### สิ่งที่ดูเหมือนจะช่วยแต่ไม่ช่วย

| | ทำไมไม่ทำ |
|---|---|
| ปิด `autoRefreshToken` ของ Supabase ตอนลงพื้นหลัง | library ทำให้อยู่แล้ว — ดัก `visibilitychange` แล้วหยุด timer เองเมื่อจอดับ |
| เปลี่ยนเป็นธีมมืด | ช่วยเฉพาะจอ OLED จอ PDA รุ่นนี้เป็น LCD ซึ่ง backlight สว่างเท่าเดิมทุกสี |
| ตัดการสั่นตอนสแกน | 1,000 ครั้ง ≈ 1.4 mAh จากแบต ~4,000 mAh คือ 0.03% แต่เสียประโยชน์ตอนไม่ได้มองจอ |
| ขอยกเว้น battery optimization | แอปไม่มีงานเบื้องหลัง ขอไปมีแต่ให้ระบบปลุกแอปฟรี ๆ |

### วัดผล

adb บนเครื่อง dev ปัจจุบันใช้ไม่ได้ (transport ตาย) จึงวัดจากตัวเครื่อง:
**ตั้งค่า → แบตเตอรี่ → การใช้งานรายแอป** หลังนับจริง 1 ชั่วโมง
จดสามค่า: % แบตที่ลด, จำนวนครั้งที่สแกน, เวลาที่จอเปิด

ถ้าซ่อม adb ได้จะละเอียดกว่ามาก:

```bash
adb shell dumpsys batterystats --charged co.anin.cyclecount
```
